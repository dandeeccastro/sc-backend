class Vacancy < ApplicationRecord
  ...
  def one_per_user
    vacancies_by_same_user = Vacancy.where('user_id = :user_id AND id != :id', { user_id: user.id, id: id })
    errors.add(:multiple_vacancies, 'Você já tem reserva nessa palestra!') unless vacancies_by_same_user.empty?
  end

  def under_talk_limit
    vacancies_by_talk = Vacancy.where('talk_id = :talk_id AND id != :id', { talk_id: talk.id, id: id })
